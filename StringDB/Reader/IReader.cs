...
namespace StringDB.Reader {
	...
	public interface IReader : IEnumerable<ReaderPair> {
		...
		bool IsIndexAfter(IReaderInteraction r, bool doSeek = true);
		...
		/// <param name="quickSeek">The location to seek to</param>
		/// <returns>True if there is an index after the index you're looking for, false if not.</returns>
		bool IsIndexAfter(string index, bool doSeek = true, ulong quickSeek = 0);
		...
		IReaderInteraction IndexAfter(IReaderInteraction r, bool doSeek = true);
		...
		/// <param name="quickSeek">The location to seek to</param>
		/// <returns>An IReaderInteraction with the information if there is stuff, null if not.</returns>
		IReaderInteraction IndexAfter(string index, bool doSeek = true, ulong quickSeek = 0);
