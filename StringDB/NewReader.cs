...
namespace StringDB {
	...
	public class NewReader : IReader {
		public NewReader(Stream streamUse) {
			_stream = streamUse;
		...
		public string GetValueOf(IReaderInteraction r, bool doSeek = false) => GetValueOf(r.Index, doSeek, r.QuickSeek);
		public string GetValueOf(string index, bool doSeek = false, ulong quickSeek = 0) => _ValueOf(index, doSeek, quickSeek);

		public bool IsIndexAfter(IReaderInteraction r, bool doSeek = false) => IsIndexAfter(r.Index, doSeek, r.QuickSeek);
		public bool IsIndexAfter(string index, bool doSeek = false, ulong quickSeek = 0) => _IsIndexAfter(index, doSeek, quickSeek);

		public IReaderInteraction IndexAfter(IReaderInteraction r, bool doSeek = false) => IndexAfter(r.Index, doSeek, r.QuickSeek);
		public IReaderInteraction IndexAfter(string index, bool doSeek = false, ulong quickSeek = 0) => _IndexAfter(index, doSeek, quickSeek);
	...
	public class ReaderEnumerator : IEnumerator<string> {
		internal ReaderEnumerator(NewReader parent, IReaderInteraction start) {
			this._parent = parent;
		...
		private NewReader _parent { get; set; }
