...
namespace StringDB.Tests.NullTests {
	public partial class InoperableWriterTests {
		...
		public void InsertRange_Null() {
			Assert.Throws<InoperableException>(() => { Writer.InsertRange((ICollection<KeyValuePair<string, string>>)null); });
			Assert.Throws<InoperableException>(() => { Writer.InsertRange((ICollection<KeyValuePair<string, byte[]>>)null); });
			Assert.Throws<InoperableException>(() => { Writer.InsertRange((ICollection<KeyValuePair<string, System.IO.Stream>>)null); });
		}

		[Fact]
		public void InsertRange_EmptyCollection() {
			Assert.Throws<InoperableException>(() => { Writer.InsertRange(new List<KeyValuePair<string, string>>() { }); });
			Assert.Throws<InoperableException>(() => { Writer.InsertRange(new List<KeyValuePair<string, byte[]>>() { }); });
			Assert.Throws<InoperableException>(() => { Writer.InsertRange(new List<KeyValuePair<string, System.IO.Stream>>() { }); });
		}
